...
**** filter-branch --tree-filter
     ...
     #+begin_src shell
       git filter-branch --tree-filter "rm -rf path_fo_file" HEAD
     #+end_src
...
**** filter-branch --index-filter
     ...
     Rewriting History, except that instead of passing a command that modifies
     files checked out on disk, you’re modifying your staging area or index each
     time.

     It works on the index, not on the disk.

     The reason to do it this way is speed – because Git doesn’t have to check
     out each revision to disk before running your filter, the process can be
     much, much faster.

     #+begin_src shell
       git filter-branch --index-filter "git rm -rf --cached --ignore-unmatch path_fo_file" HEAD
     #+end_src
